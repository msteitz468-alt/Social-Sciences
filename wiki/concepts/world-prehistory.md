---
title: "World Prehistory"
concept_type: analytical
coined_by: ["[[clark-grahame]] (as textbook genre / research frame; earlier local uses of the phrase)"]
date_coined: "1961 (*World Prehistory: An Outline*) as the first widely used modern global textbook frame"
disciplines: [archaeology, anthropology]
emic_or_etic: etic
operationalized_as: "A single chronological-ecological narrative covering the prehistoric human past of all inhabited continents, treating regional sequences comparatively rather than as a Europe- or Near-East-only story"
contested: yes
related_concepts: ["[[prehistory]]", "[[economic-prehistory]]", "[[neolithic-revolution]]", "[[urban-revolution]]", "[[ex-oriente-lux]]", "[[global-colonization]]", "[[exaptation]]"]
sources_ingested: 6
last_updated: 2026-07-10
tags: [concept, archaeology]
---

# World Prehistory

## Definition as Coined

**World prehistory** names the project of writing the human past before continuous written records **as a single global field** — not a patchwork of national or continental antiquities. In this wiki the load-bearing coining text is [[clark-grahame|Grahame Clark]]'s *[[clark-world-prehistory-1961|World Prehistory: An Outline]]* (1961): a brief outline of "man's prehistoric past" across the Old World, the New World, Australasia, and the Pacific, framed by Pleistocene environment, hominin evolution, and successive ecological adaptations (attributed, Clark 1961 preface and Introduction).

Clark's own earlier program ([[economic-prehistory]]) supplied the method; the 1961 book supplied the **geographical scope**. Histories of the discipline (e.g. Trigger) later treat Clark's *World Prehistory* series (1961; rev. 1969, 1977) as the first modern-style global prehistory textbook.

## Semantic History

- **Pre-1961:** regional culture-histories (e.g. Childe on Europe and the Near East) and popular Old-World "rise of civilization" narratives dominated English-language synthesis; the Americas, Australia, and much of sub-Saharan Africa were often residual chapters or omitted.
- **Clark 1961:** equal structural commitment to New World and Australasian chapters; early radiocarbon used to align regional sequences into one outline; farming and urbanization still Near-East-centric in origin story but explicitly multi-regional in later spread.
- **Post-1961 (as of later textbooks and archaeogenetics):** successive surveys (Wenke, Fagan, Scarre, *Cambridge World Prehistory*, etc.) normalize the global frame; absolute chronology and multi-proxy evidence have repeatedly revised the sequences Clark sketched (attribute each revision to its source when ingested).

## Operationalizations

Clark (attributed, 1961): (1) start from physical environment and biological evolution; (2) treat **culture** (learned, traditional behaviour) as the criterion that makes a hominid human; (3) follow successive subsistence adaptations — hunting–gathering, farming, urban civilization — by region; (4) end each regional story at the onset of continuous written history or colonial contact, so "prehistory" is time-transgressive (see [[prehistory]]).

## Applications

Organizes comparative deep-time teaching and the wiki's cross-regional culture/site network; anchors reflexivity about which regions get narrative depth in mid-century vs. contemporary syntheses.

## Critiques and Limitations

Uneven research intensity and authorial reading produce geographic distortion (Clark's own preface caveat); sub-Saharan Iron Age and African complexity were thin by later standards (Trigger notes slighting of sub-Saharan Africa in the 1961 edition); progressivist and adaptive-race language requires attribution; farming/urban "origins" still largely [[ex-oriente-lux]] in structure even while the book's map is global.

### Gamble — *Timewalkers* (1994): colonization as organizing question

[[gamble-clive|Clive Gamble]]'s *[[gamble-timewalkers-1994|Timewalkers]]* retools world prehistory around **[[global-colonization]]** — *why were people everywhere?* — rather than *why was civilization not universal?* (attributed, preface and ch. 1). He credits Clark with showing that a world prehistory is possible and profitable, and quotes Clark that prehistory properly apprehended gives a more valid perspective on the contemporary situation than parochial histories (attributed, ch. 11). Gamble's program: purpose without progress; [[exaptation]] vs adaptation; Ancients/Pioneers/Moderns range-phases; critique of double standards that naturalize deep prehistory while reserving "history" for post-agricultural societies (legacy of [[childe-v-gordon|Childe]]'s Neolithic Revolution framing — attributed). Goal of world prehistory should be diversity and common humanity, not an origins quest that establishes "the antiquity of exclusion" (attributed, ch. 11). Empirical horizon is mid-1990s (pre-aDNA).

### Wenke & Olszewski — *Patterns in Prehistory* (2007)

[[wenke-olszewski-patterns-in-prehistory-2007|Wenke & Olszewski]]'s fifth edition is a mid-2000s **world-prehistory textbook** in the post-Clark genre: global chronological-ecological narrative organized around six themes — intellectual background; archaeological methods; culture; emergence of anatomically modern humans; food-producing economies; and complex societies, civilizations, and states (attributed, preface). The overview timeline they present (as of 2007) anchors first stone tools ~2.6 Ma in Africa, *Homo* ~2.5 Ma, *Homo erectus*-grade out-of-Africa ~1.7 Ma, early modern humans in Africa ~160 ka, Americas colonization beginning ~20–15 ka, and multi-regional agricultural origins (e.g. Middle East ~8500 B.C., Mesoamerica ~8000 B.C.) — textbook round numbers, not lab intervals (attributed).

They frame contemporary archaeology's "grand questions" as world-prehistory problems: why, after ~2.5 million years as hunter-foragers, did some ancestors "suddenly" become farmers after ~10,000 years ago; why class-based ancient civilizations and large cities appeared in some regions but not others (aboriginal Australia, North America, England, Hawaii as their negative cases) (attributed, ch. 1). Independent multi-center domestication and multi-center [[pristine-state|primary]] complexity are treated as the explanatory core of the global story, not single-origin diffusion alone (attributed). Methodologically they endorse a "holistic"/eclectic synthesis of processual ecological constraints with post-processual concern for tradition, ideas, gender, power, and ritual, while insisting not all interpretations are equally valid (attributed). **Horizon note:** genetics, peopling, and human-origins sections reflect 2007 textbook consensus — keep those claims attributed and date-stamped; do not promote them as current aDNA consensus.

## After the Ice densification (Mithen 2003)

*(Attributed — [[mithen-steven]], *[[mithen-after-the-ice-2003|After the Ice]]* 2003 — a post-Clark popular world-prehistory narrative.)*

- **Remit:** if ~50,000 BC marks the “birth of history” (modern mind), **20,000–5000 BC** is its “coming of age” — farming, towns, civilization originate in this window (attributed, ch. 1).
- **Method of global narrative:** without writing, history is written from archaeological rubbish, environmental proxies (pollen, beetles), and occasional modern genes/languages (attributed) — four-way non-identity still required: material ≠ language ≠ genes ≠ peoples.
- **Structure:** starts Fertile Crescent, then multi-continental survey (Europe Mesolithic–Neolithic, Americas peopling, Australia/New Guinea, East Asia, Africa) using a fictional Victorian “Lubbock” observer as literary device — reconstructions are narrative, not observation (flag on all site scenes).
- **Chronology practice:** all dates given as **calendar years BC**; endnotes hold raw ¹⁴C and 1σ calibrations — explicit calibration hygiene for a global outline (attributed, ch. 2).
- **Genre position:** densifies Clark’s global textbook frame with post-processual-friendly ideology/symbol chapters and mid-1990s–early-2000s site literature; **pre-aDNA** genetics and peopling claims must stay date-stamped “(as of Mithen 2003).”

Source: [[mithen-after-the-ice-2003]].

### Bogucki — *The Origins of Human Society* (1999)

[[bogucki-peter|Peter Bogucki]]'s Blackwell History of the World volume (*[[bogucki-origins-human-society-1999|The Origins of Human Society]]*, 1999) is a **global comparative synthesis** in the post-Clark textbook/series genre, but with an explicit **individual self-interest** theoretical slant rather than a pure systems or culture-history frame. Scope: early hominids through early states and “chiefdoms in the shadow of states.” Five recurring themes (attributed, Ch.1): technology–society interplay; self-interested decision-makers under risk/uncertainty; household-scale behavioral trajectories; humans in multi-dimensional landscapes; intellectual history of prehistoric archaeology as meta-discipline.

Load-bearing analytic sequence for the Holocene half: breakdown of [[pleistocene-band-society]] → late-forager proto-households and agricultural commitment → [[transegalitarian]] competition ([[aggrandizer]] + residual drop-down) → unstable chiefdoms → tributary states and shadow chiefdoms. **Horizon note:** late-1990s research (pre-aDNA; peopling and phylogeny sections age faster than the household/inequality apparatus). Companion series volume on *Beginnings of Civilization* was planned for Robert Wenke.

### Scarre (ed.) — *The Human Past* (2013)

[[scarre-human-past-2013|Scarre (ed.), *The Human Past*]] (3rd ed. 2013, Thames & Hudson) is the most-used current university survey in the post-Clark genre: a **multi-author** textbook, edited by [[scarre-chris|Chris Scarre]] with region-specialist chapters (Klein on African origins; Connah on Holocene Africa; Higham on East/Southeast Asia; Coningham on South Asia; Milner and Wills on North America; among others). Structure: Part I (human evolution in Africa and the peopling of the Old World, ~6 Ma–11,500 years ago) then Part II, a region-by-region tour of the Holocene — origins of food production and the rise of complex societies across Southwest Asia, East and Southeast Asia, South Asia, Africa, Europe, the Mediterranean, Mesoamerica, South America, and North America. Its closing synthesis (attributed, Scarre ed. 2013, ch. 19) organizes the whole global story around four long-run themes: **demographic increase, agricultural intensification and environmental degradation, biological exchange** (culminating in the Columbian exchange and post-contact disease mortality), and **climate change** as a recurrent backdrop rather than a prime mover.

Because it is multi-author and peer-taught, it carries presumptive weight on the **state of the field** region by region, but is downstream of the primary studies it summarizes — keep single findings attributed and date-stamp its fast-moving science "(as of Scarre ed. 2013)". Its chief additions to this wiki's coverage were on the **Andean sequence** ([[caral]], [[chavin-culture]], [[moche-culture]], [[nazca-culture]], [[wari-culture]], [[tiwanaku-culture]], [[chimu-culture]], [[inca-empire]]; [[andean-chronology]]) and East/Southeast Asia ([[yayoi-culture]], [[angkor]]).
